...
// bootTimeOffset lazily estimates the boot time offset on first use. Lazy
// evaluation avoids crashing any binary that merely imports this package at
// init time; an estimation failure only surfaces (as a panic) when the
// offset is actually needed to convert event timestamps.
var bootTimeOffset = sync.OnceValue(func() int64 {
