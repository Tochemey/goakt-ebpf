...
// Exists returns true if the process is still running. Uses kill(pid, 0) for a
// zero-allocation check. Safe to call frequently (e.g. from a watch loop).
func (id ID) Exists() bool {
	if id < 0 {
		return false
	}
	return syscall.Kill(int(id), syscall.Signal(0)) == nil
}
