...
// HandlerShutdown is implemented by handlers that support graceful shutdown
// (e.g. flushing pending spans to the exporter).
type HandlerShutdown interface {
...
// Shutdown shuts down all handlers that implement HandlerShutdown (e.g. to
// flush pending telemetry to the exporter). Call this before process exit for
// clean shutdown.
func (h *Handler) Shutdown(ctx context.Context) error {
	var err error
	if s, ok := h.TraceHandler.(HandlerShutdown); ok {
		err = errors.Join(err, s.Shutdown(ctx))
	}
	if s, ok := h.MetricHandler.(HandlerShutdown); ok {
		err = errors.Join(err, s.Shutdown(ctx))
	}
	if s, ok := h.LogHandler.(HandlerShutdown); ok {
		err = errors.Join(err, s.Shutdown(ctx))
	}
	return err
}
